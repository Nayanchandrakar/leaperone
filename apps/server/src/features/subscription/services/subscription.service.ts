...
import { CHECKOUT_STATUSES } from "@/features/subscription/constants/subscription.constants"
import { SubscriptionUtils } from "@/features/subscription/utils/subscription.utils"
import type {
  CheckoutSession,
...
export class SubscriptionService {
  async constructWebhook(c: StripeContext) {
  ...
  async billingPortal(c: CheckoutSessionContext) {
    ...
    const billingPortal = await stripe.billingPortal.sessions.create({
      ...
      return_url: RouteUtils.createAbsoluteRoute("/dashboard"),
    })

...
type ContentEditorActions = {
  ...
  updateSectionField: (sectionIdx: number, path: string[], value: any) => void
...
// Selectors
export const useContentSections = () => useContentEditorStore((state) => state.sections)
export const useContentSection = (idx: number) =>
  useContentEditorStore((state) => state.sections[idx])
export const useContentTemplateId = () => useContentEditorStore((state) => state.templateId)

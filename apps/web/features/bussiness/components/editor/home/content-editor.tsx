...
const ContentEditor = () => {
  ...
  const renderSections = useCallback((type: ContentSectionType, index: number) => {
    const Component = CONTENT_FORMS[type]
  ...
  return (
    <EditorBlock>
      <SortableList
        ...
        renderItem={({ type }, index) => renderSections(type, index)}
      />
    </EditorBlock>
  )
}
